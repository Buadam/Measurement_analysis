...
set(hFig, 'Position', [10 1 17.2 11]);
...
for t=1:6
    ...
    if t==6
        ...
        tau_cycsky_logmean(1)=10^(mean(log10(tau(1:4))));   
        tau_skyfm_mean(1)=mean(tau(7:end));
        tau_skyfm_logmean(1)=10^(mean(log10(tau(7:end)))); 
       
    else
        tau_cycfm_mean(t)=mean(tau);
        tau_cycfm_logmean(t)=10^(mean(log10(tau)));
...
for t=7:13
    ...
    tau_cycsky_logmean(t-5)=10^(mean(log10(tau)));
    ...
    tau_skyfm_logmean(t-5)=10^(mean(log10(tau)));
end
...
set(gca,'YScale','log')
%ylim([1e-4,1e2])
Yt=[1e-4,1e-3,1e-2,1e-1,1,1e1,1e2];
set(gca,'YTick',Yt)
%plot(T(1:6),tau_cycfm_mean,'ko-')
plot(T(1:5),tau_cycfm_logmean,'ko-')
%plot(T(7:13),tau_cycsky_mean,'ro-')
plot(T(6:13),tau_cycsky_logmean,'ro-')
%plot(T(7:13),tau_skyfm,'bo-')
plot(T(6:13),tau_skyfm_logmean,'bo-')