function OUTPUT = complexsusc_logtau(p,omega)
tau=10^(p(2));
...
if ~isempty(find(omega==0)) %if zero frequency is included in the list
    ...
    imchi(zeroind)=0;
    rechi(zeroind)=p(4)+p(5);
   

    imchi(ind)=(p(1)*cos(p(3)*pi/2)./((omega*tau).^(p(3)-1)+(omega*tau).^(1-p(3))+2*sin(p(3)*pi/2)))';
    rechi(ind)=p(5)+(p(4)*((omega*tau).^(p(3)-1)+sin(p(3)*pi/2))./((omega*tau).^(p(3)-1)+(omega*tau).^(1-p(3))+2*sin(p(3)*pi/2)))';
    OUTPUT=[imchi',rechi'];
else
    imchi=(p(1)*cos(p(3)*pi/2)./((omega*tau).^(p(3)-1)+(omega*tau).^(1-p(3))+2*sin(p(3)*pi/2)))';
    rechi=p(5)+(p(4)*((omega*tau).^(p(3)-1)+sin(p(3)*pi/2))./((omega*tau).^(p(3)-1)+(omega*tau).^(1-p(3))+2*sin(p(3)*pi/2)))';
    OUTPUT=[imchi,rechi];
