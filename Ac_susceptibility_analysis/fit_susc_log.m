function [ReChi,ImChi,param]=fit_susc_log(chi_B_T,abs_B_T,equal)
  ...
  if equal==0  
        p0=[Chi0_im/rescale_im,log10(tau),alpha,Chi0_re/rescale_re,Chi0_re_off/rescale_re];
        ChiComp=@(p3,omega)complexsusc_logtau(p3,omega);
        ...
        CompChi=complexsusc_logtau(param,2*pi*fcont); %Im(Chi) and Re(Chi) are fitted together
        ImChi=CompChi(:,1);
        ReChi=CompChi(:,2);
  else
        p0=[Chi0_im/rescale_im,log10(tau),alpha,Chi0_re_off/rescale_re];
        resc=rescale_im/rescale_re;
        ChiComp=@(p3,omega)complexsusc_eq_logtau(p3,omega,resc);
