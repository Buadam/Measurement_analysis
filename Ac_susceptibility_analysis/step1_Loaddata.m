...
%{
T11rso=dlmread(fullfile(Folder_dc,'GaVS_TR234_tf_11.rso.dat'),',',32,0);
data11dc=[T11rso(:,3) T11rso(:,5)];
[dmdh11(:,1),dmdh11(:,2)]=centdiff(data11dc);

T11p5rso=dlmread(fullfile(Folder_dc,'GaVS_TR234_tf_11p5.rso.dat'),',',32,0);
data11p5dc=[T11p5rso(:,3) T11p5rso(:,5)];
[dmdh11p5(:,1),dmdh11p5(:,2)]=centdiff(data11p5dc);
%}
freq=7;
...
for t=1:numel(T)
    ...
    if ~isempty(dmdhc{t})
        dmdhcent=dmdhc{t};
        h_dc=plot(dmdhcent(2:end,1),dmdhcent(2:end,2)+(T(t)-T(1))*shift,'o-','MarkerSize',4,'Color',[0.3,0.3,0.3]);
    end
end
