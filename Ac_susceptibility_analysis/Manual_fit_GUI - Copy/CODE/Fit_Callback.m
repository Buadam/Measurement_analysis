...
global chi_B_T abs_B_T fcont N g_HN g_CC g_RE g_IM ImChi_arb ReChi_arb ReChi1 ReChi2 ImChi1 ImChi2 ReChi_Re ReChi_Im ImChi_Re ImChi_Im A
...
%Fitting
 ...
 if CC==1
        ...
        taumin=str2double(get(handles.taumin,'String'));
        taumax=str2double(get(handles.taumax,'String'));
        tauD=logspace(log10(taumin),log10(taumax),N+1);
...
if IM==1
        [p2,resnorm3,residual3,exitflag3,output3]  = lsqcurvefit(ChiComp4,p0_1,omega,abs_B_T'/rescale_im,lb1,ub1);
        ChiComp5=@(p3,omega)complexsusc_offset(p3,omega,[p2(1),p2(2),p2(3)/rescale_re*rescale_im]);
        p6=lsqcurvefit(ChiComp5,p1(4),omega,chi_B_T'/rescale_re,0,1);
        param4=[p2(1),p2(2),p2(3)*rescale_im,p6*rescale_re];
        ...
        set(handles.ImChi,'Nextplot','add')
        set(handles.Taudist,'Nextplot','add')
        plot(handles.ReChi,fcont,ReChi_Im,'y--')
        plot(handles.ImChi,fcont,ImChi_Im,'y--')
        plot(handles.Taudist,tauD,g_IM,'yo-')
        set(handles.ReChi,'Nextplot','replace')
        set(handles.ImChi,'Nextplot','replace')
        set(handles.Taudist,'Nextplot','replace')
 end 
...
box on
set(gca,'XScale','log')
da=plot(fr,chi_B_T/m*M,'o','Color','k');
ar=plot(fcont,ReChi_arb/m*M,'-','Color','k');
cc=plot(fcont,ReChi1/m*M,'-','Color','b');
hn=plot(fcont,ReChi2/m*M,'--','Color','r');
re=plot(fcont,ReChi_Re/m*M,'-','Color','g');
im=plot(fcont,ReChi_Im/m*M,'-','Color','y');

legend([da,ar,cc,hn,re,im],'Measured data','Arbitrary g(\tau)','Cole-Cole','Havriliak-Negami','\chi^{,} Fitted','\chi^{,,} Fitted','FontSize',8) 
set(gca,'FontSize',8);
...
box on
set(gca,'XScale','log')
da=plot(fr,abs_B_T/m*M,'o','Color','k');
ar=plot(fcont,ImChi_arb/m*M,'-','Color','k');
cc=plot(fcont,ImChi1/m*M,'-','Color','b');
hn=plot(fcont,ImChi2/m*M,'--','Color','r');
re=plot(fcont,ImChi_Re/m*M,'-','Color','g');
im=plot(fcont,ImChi_Im/m*M,'-','Color','y');

set(gca,'FontSize',8);

%legend([da,ar,cc,hn,re,im],'Measured data','Arbitrary g(\tau)','Cole-Cole','Havriliak-Negami','\chi^{,} Fitted','\chi^{,,} Fitted') 
subplot(2,2,[3,4])
cla
hold all
box on
...
for t=1:length(tauD)-1  
    tau=[tau logspace(log10(tauD(t)),log10(tauD(t+1)),Ntau)];
    g_arb=[g_arb A(t)*ones(1,Ntau)];
end

%renormalize amplitudes
I=trapz(log(tau),g_arb,2);


ar=plot(tau,g_arb/I,'-','Color','k');
cc=plot(tauD,g_CC,'-','Color','b');
hn=plot(tauD,g_HN,'--','Color','r');
re=plot(tauD,g_RE,'-','Color','g');
im=plot(tauD,g_IM,'-','Color','y');
...
%legend([ar,cc,hn,re,im],'Arbitrary g(\tau)','Cole-Cole','Havriliak-Negami','\chi^{,} Fitted','\chi^{,,} Fitted') 
addpath('G:\Adam\Install\Mathworks.Matlab.R2015a.x64.ISO-TBE\altmany-export_fig-2763b78\altmany-export_fig-2763b78')

%export_fig(hFig,'nondebye_9K_26mT_single.eps') 
