...
global chi_B_T abs_B_T N A fcont ImChi_arb ReChi_arb ReChi1 ReChi2 ImChi1 ImChi2 g_CC g_HN
...
omega=2*pi*fcont;
ImChi_arb=zeros(size(omega))'; 
ReChi_arb=zeros(size(omega))';

chi_inf_re=get(handles.chi_inf,'Value')*1e-4;
tau_seg=logspace(log10(taumin),log10(taumax),N+1);
...
for t=1:length(tau_seg)-1 
    ...
    ImChi_arb=ImChi_arb-A(t)*imag(chi);
    ReChi_arb=ReChi_arb+A(t)*real(chi);
    
    plot(handles.ReChi,fcont,A(t)*real(chi),'r-')
    plot(handles.ImChi,fcont,-A(t)*imag(chi),'r-')
    
end
ReChi_arb=ReChi_arb+chi_inf_re;



plot(handles.ReChi,fcont,ReChi_arb,'k-')
plot(handles.ImChi,fcont,ImChi_arb,'k-')


I=trapz(log(tau_seg(2:end)),A,2);

plot(handles.Taudist,tau_seg(2:end),A/I,'ko-')
